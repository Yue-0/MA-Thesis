...
% 英文目录：使用自定义的.entoc
\makeatletter
\def\englishtableofcontents{%
  \begingroup
    \renewcommand{\contentsname}{\xiaosan\Cambriab{\textbf{\hfill TABLE OF CONTENTS\hfill}}\vspace*{-1.25em}}  % 英文目录标题
...
% 英文目录项：\addenglish{层级}{标题}
\newcommand{\englishtitle}[2]{
  \addcontentsline{entoc}{#1}{\protect\numberline{\csname the#1\endcsname}#2}
}


\renewcommand\listfigurename{\hfill 图目录 \hfill}
\renewcommand\listtablename{\hfill 表目录 \hfill}
\renewcommand{\bibname}{\hfill 参~考~文~献 \hfill}

% 主要符号表
\def\symboltable{
  \begin{center}\xiaosan\hei 主要符号表\end{center}
  ...
  \begin{table}[ht]
    ...
    \begin{tabularx}{\linewidth}{>{\centering\arraybackslash}X>{\centering\arraybackslash}X>{\centering\arraybackslash}X}
        \toprule
        \textbf{符~~号}   & \textbf{代表意义} & \textbf{单位或定义}     \\
        \midrule
        \textbf{英文字母}                                             \\
        $\boldsymbol{v}$ & 速度              & m/s                    \\
        $\boldsymbol{a}$ & 加速度            & m/s\textsuperscript{2}  \\
        \textbf{希腊字母}                                              \\
        $\omega$         & 角速度            & rad/s                   \\
        $\alpha$         & 角加速度          & rad/s\textsuperscript{2} \\
        \textbf{无量纲量}                                               \\
        $\pi$            & 圆周率            & \textbackslash           \\
        $\mathrm{e}$     & 自然对数的底数     & \textbackslash           \\
        \textbf{下标}                                         \\
        $\boldsymbol{x}_i$ & 向量$\boldsymbol{x}$的第$i$个分量 &\\
        \textbf{缩写}                                          \\
        \bottomrule
    \end{tabularx}
  \end{table}
...
\renewcommand{\cftfigpresnum}{\figurename\ }  % 图目录编号前加“图 ”
\setlength{\cftfignumwidth}{3em}  % 加宽编号区域
\renewcommand{\cfttabpresnum}{\tablename\ }  % 表目录编号前加“表 ”
\setlength{\cfttabnumwidth}{3em}  % 加宽编号区域

\newcommand{\figcaption}[3][]{
  \bicaption[#1]{#2}{#2}{Fig.}{#3}
}
...
\def\originality{
  ...
  \begin{center}
    ...
    {\xiaoer\hei\centerline{学位论文原创性声明}}
    ...
    {\xiaosi\song 学{\hfill}位{\hfill}论{\hfill}文{\hfill}题{\hfill}目{\hfill}：\fixedunderlinecenter[12.6cm]{\@ctitle}}
    \parbox[t][1.22cm][c]{\textwidth}
    {\xiaosi\song 作{\hfill}者{\hfill}签{\hfill}名{\hfill}：\fixeddistanceleft[12.6cm]{\underline{\hspace{5.8cm}}\hfill
...
\def\authorization
{
  ...
  \begin{center}
    \parbox[t][1.52cm][c]{\textwidth}
    {\xiaoer\hei\centerline{大连理工大学学位论文版权使用授权书}}
    \parbox[t][5.68cm][c]{\textwidth}
    {
      \xiaosi\song\noindent
    ...
    {\xiaosi\song 作{\hfill}者{\hfill}签{\hfill}名{\hfill}：\fixeddistanceleft[12.6cm]{\underline{\hspace{5.8cm}}\hfill
	  日期：\underline{\hspace{1.4cm}}~年~{\underline{\hspace{0.7cm}}~月~{\underline{\hspace{0.7cm}}~日~}}}}
    \parbox[t][0.9cm][c]{\textwidth}
    {\xiaosi\song 导{\hfill}师{\hfill}签{\hfill}名{\hfill}：\fixeddistanceleft[12.6cm]{\underline{\hspace{5.8cm}}\hfill
	日期：\underline{\hspace{1.4cm}}~年~{\underline{\hspace{0.7cm}}~月~{\underline{\hspace{0.7cm}}~日~}}}}
  \end{center}
  \renewcommand{\baselinestretch}{1.27}
}
